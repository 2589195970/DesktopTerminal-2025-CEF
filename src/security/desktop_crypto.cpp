#include "desktop_crypto.h"

#include <QCryptographicHash>
#include <QMessageAuthenticationCode>
#include <QRandomGenerator>

namespace {

const unsigned char SBOX[256] = {
    
};

const unsigned char RCON[11] = {0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36};

void keyExpansion(const unsigned char *key, unsigned char roundKeys[176])
{
    for (int i = 0; i < 16; ++i) {
        roundKeys[i] = key[i];
    }
    for (int i = 4; i < 44; ++i) {
        unsigned char temp[4];
        for (int j = 0; j < 4; ++j) {
            temp[j] = roundKeys[(i - 1) * 4 + j];
        }
        if (i % 4 == 0) {
            unsigned char t = temp[0];
            temp[0] = SBOX[temp[1]] ^ RCON[i / 4];
            temp[1] = SBOX[temp[2]];
            temp[2] = SBOX[temp[3]];
            temp[3] = SBOX[t];
        }
        for (int j = 0; j < 4; ++j) {
            roundKeys[i * 4 + j] = roundKeys[(i - 4) * 4 + j] ^ temp[j];
        }
    }
}

void addRoundKey(unsigned char state[16], const unsigned char *roundKey)
{
    for (int i = 0; i < 16; ++i) {
        state[i] ^= roundKey[i];
    }
}

void subBytes(unsigned char state[16])
{
    for (int i = 0; i < 16; ++i) {
        state[i] = SBOX[state[i]];
    }
}

void shiftRows(unsigned char state[16])
{
    unsigned char temp;
    temp = state[1]; state[1] = state[5]; state[5] = state[9]; state[9] = state[13]; state[13] = temp;
    temp = state[2]; state[2] = state[10]; state[10] = temp; temp = state[6]; state[6] = state[14]; state[14] = temp;
    temp = state[15]; state[15] = state[11]; state[11] = state[7]; state[7] = state[3]; state[3] = temp;
}

unsigned char xtime(unsigned char x)
{
    return static_cast<unsigned char>((x << 1) ^ (((x >> 7) & 1) * 0x1b));
}

void mixColumns(unsigned char state[16])
{
    for (int i = 0; i < 4; ++i) {
        unsigned char *col = state + i * 4;
        unsigned char a = col[0], b = col[1], c = col[2], d = col[3];
        col[0] = xtime(a) ^ xtime(b) ^ b ^ c ^ d;
        col[1] = a ^ xtime(b) ^ xtime(c) ^ c ^ d;
        col[2] = a ^ b ^ xtime(c) ^ xtime(d) ^ d;
        col[3] = xtime(a) ^ a ^ b ^ c ^ xtime(d);
    }
}

void encryptBlock(unsigned char block[16], const unsigned char roundKeys[176])
{
    addRoundKey(block, roundKeys);
    for (int round = 1; round < 10; ++round) {
        subBytes(block);
        shiftRows(block);
        mixColumns(block);
        addRoundKey(block, roundKeys + round * 16);
    }
    subBytes(block);
    shiftRows(block);
    addRoundKey(block, roundKeys + 160);
}

QByteArray pkcs7Pad(const QByteArray &input, int blockSize)
{
    int padLen = blockSize - (input.size() % blockSize);
    QByteArray output = input;
    output.append(QByteArray(padLen, static_cast<char>(padLen)));
    return output;
}

QByteArray aesEncryptCbc(const QByteArray &plainText, const QByteArray &keyBytes, const QByteArray &iv)
{
    if (keyBytes.size() != 16 || iv.size() != 16) {
        return QByteArray();
    }

    unsigned char roundKeys[176];
    keyExpansion(reinterpret_cast<const unsigned char *>(keyBytes.constData()), roundKeys);
    QByteArray padded = pkcs7Pad(plainText, 16);
    QByteArray cipherText;
    cipherText.resize(padded.size());
    QByteArray previousIv = iv;

    for (int offset = 0; offset < padded.size(); offset += 16) {
        unsigned char block[16];
        for (int i = 0; i < 16; ++i) {
            block[i] = static_cast<unsigned char>(padded[offset + i] ^ previousIv[i]);
        }
        encryptBlock(block, roundKeys);
        for (int i = 0; i < 16; ++i) {
            cipherText[offset + i] = static_cast<char>(block[i]);
        }
        previousIv = cipherText.mid(offset, 16);
    }
    return cipherText;
}

} // namespace

QString DesktopCrypto::encryptAesCbcBase64(const QString &plainText, const QString &base64AesKey)
{
    if (plainText.isEmpty() || base64AesKey.isEmpty()) {
        return QString();
    }

    const QByteArray keyBytes = QByteArray::fromBase64(base64AesKey.toUtf8());
    if (keyBytes.size() != 16) {
        return QString();
    }

    QByteArray iv = randomBytes(16);
    QByteArray cipherText = aesEncryptCbc(plainText.toUtf8(), keyBytes, iv);
    if (cipherText.isEmpty()) {
        return QString();
    }

    return QString::fromUtf8((iv + cipherText).toBase64());
}

QString DesktopCrypto::hmacSha256Hex(const QString &payload, const QString &secret)
{
    QMessageAuthenticationCode code(QCryptographicHash::Sha256);
    code.setKey(secret.toUtf8());
    code.addData(payload.toUtf8());
    return QString::fromLatin1(code.result().toHex());
}

QByteArray DesktopCrypto::randomBytes(int length)
{
    QByteArray bytes(length, 0);
    for (int i = 0; i < length; ++i) {
        bytes[i] = static_cast<char>(QRandomGenerator::global()->generate() & 0xFF);
    }
    return bytes;
}
